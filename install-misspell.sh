...
tag_to_version() {
  ...
  github_release_temp "$OWNER/$REPO" "${TAG}"

  REALTAG=$(github_release "$OWNER/$REPO" "${TAG}") && true
  echo "REALTAG=${REALTAG}"
...
github_release_temp() {
  ...
  giturl="https://github.com/${owner_repo}/releases/${version}"
  tmp=$(mktemp)
  curl -vvv -sL -H "Accept:application/json" -o "$tmp" "$giturl"
#  curl -vvv -w '%{http_code}' -sL -H "Accept:application/json" -o "$tmp" "$giturl" || true
  echo "YOYOYOYOYO"
  cat $tmp
}
hash_sha256() {
